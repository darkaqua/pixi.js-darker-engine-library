...
export const displayObjectHitBoxSystem = async () => {
  ...
  const onAdd = async (entityId: number) => {
    ...
    const [graphics] = await Library.engine.addEntity(
      graphicsPolygonEntity({
        childOf: entityId,
    ...
    const targetEntity = targetEntityId
      ? Library.engine.getEntity(targetEntityId)
      : entity;
    ...
    clickEventListenerEntityMap[entityId] = Utils.render.addEventListener(
      ...
      async () => {
        if (removeCurrentSelections) {
          let entityList = Library.engine.getEntityListByComponents(
